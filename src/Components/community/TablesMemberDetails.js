...
const TablesMemberDetails = (props) => {
  ...
  console.log("newmemberdata", props.renderData);
  console.log("currentidnew", props.Id);
  useEffect(() => {
    props.renderData.forEach((ele) => {
      if (ele._id === props.Id) {
        setnewmembDetails(ele);
      }
    });
  }, [props.Id, props.renderData]);
  console.log("newmemberdata", newmembDetails);
  ...
  return (
    <Surrounding>
      <Aside>
        <img
          className="cross"
          onClick={() => props.setNewMemberDetails(!props.newMemberDetails)}
          src={img.cancel}
          alt="cancelimg"
        />
