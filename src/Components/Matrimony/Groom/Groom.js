...
const Groom = () => {
  ...
  useEffect(() => {
    const getBrideList = async () => {
      const res = await axios.get(
        "https://sgvr-server.herokuapp.com/api/admin/profile?gender=male",
        {
          headers: {
            "x-access-token":
    ...
    getBrideList()
      .then((res) => {
        if (res.status === 200) {
          // grooms = res.data.data;
          setgroomlist(res.data.data);
          setloader(false);
        } else {
  ...
  useEffect(() => {
    const getBrideList = async () => {
      const res = await axios.get(
        `https://sgvr-server.herokuapp.com/api/matrimony/search?gender=male&keyword=${search}&status=${
            ...
            {groomlist.map((items, index) => (
              <tr key={index}>
                ...
                <td>
                  <SvgIconsDiv
                    onClick={() =>
                      navigater(`/matrimonylist/groom/${items._id}`)
                    }
                  >
                    <SvgIcon>
