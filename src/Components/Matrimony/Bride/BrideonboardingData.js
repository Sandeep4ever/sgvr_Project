...
const BrideboardingData = () => {
  ...
  let dateMonth = (dob) => {
    ...
    var d = new Date(dob);
    return (
      d.getUTCDate() + " " + month[d.getUTCMonth()] + " " + d.getUTCFullYear()
    );
  };
  ...
  const statusChange = async () => {
    const call = await axios.patch(
      ...
      { id: datad.createdBy },
      {
        headers: {
          "x-access-token":
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjoiNjFhZjRmNzhiOTRmNTAxMTA5MDEzOTM1IiwidXNlclR5cGUiOiJVIiwiaWF0IjoxNjQzMjg0NjAyLCJleHAiOjE2NDU4NzY2MDJ9.F7YE0A_snoJVO3TFKuvsA_bD7nm7U5hA4G47u8wLX3s",
        },
      }
    );
    return call;
...
export default BrideboardingData;
